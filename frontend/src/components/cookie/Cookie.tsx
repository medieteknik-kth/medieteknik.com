...
export default function CookiePopup({ params: { language } }: { params: { language: string }}) {
  ...
  return (
    <div className={`w-full h-screen fixed grid place-items-center top-0 bg-black/30 z-50`}>
      <div className='w-full sm:w-[512px] h-[512px] fixed bg-white border-2 border-black/50 rounded-2xl'>
        <div className='flex flex-col mx-8 h-full justify-between'>
          ...
          <div className='h-5/6 flex flex-col items-start justify-evenly'>
            ...
            }}>{t('btn_personalize')}</button>
            <button className='w-full h-16 border-2 border-black bg-white text-black rounded-2xl' onClick={() => {
              saveSettings()
              setShowPopup(false)
      ...
      {showDetailedPopup && <DetailedCookiePopup params={{ language, popup:setShowPopup }} />}
    </div>
  )
}