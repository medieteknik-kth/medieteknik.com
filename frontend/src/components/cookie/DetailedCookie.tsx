...
export default function DetailedCookiePopup({ params: { language, popup } }: { params: { language: string, popup: Dispatch<SetStateAction<boolean>> }}) {
  ...
  return (
    <div className='w-1/4 h-5/6 fixed top-20 rounded-3xl overflow-hidden bg-white border-r-2 border-black'>
      <div className='w-full flex items-center py-4 border-b-2 border-black/50 mb-4 pl-8'>
        ...
        <button className='w-8 h-8 ml-auto mr-8' onClick={() => {
      ...
      <div className='w-full h-5/6 px-8 overflow-y-scroll absolute flex flex-col'>
        <div className='w-full mb-4'>
          ...
          <div className='w-2/5 flex justify-between my-2'>
            <Link href='/privacy#policy' className='text-blue-500'>{cookieTranslation('privacyPolicy')}</Link>
            <Link href='/privacy#cookies'>{cookieTranslation('cookiePolicy')}</Link>
          </div>
        ...
        {availableCookies.map((key, index) => {
          const cookie = key as 'NECESSARY' | 'FUNCTIONAL' | 'ANALYTICS' | 'PERFORMANCE' | 'ADVERTISING'
          return (
            <div key={index} className='w-full flex flex-col mt-8 border-b-2 border-black/25 pb-4'>
              <div className='w-full flex justify-between '>
                <div className='w-fit flex items-center'>
                  <button className='w-8 h-8' type='button'>
                    {dropdowns[cookie] ?
                      <MinusIcon className='w-8 h-8' onClick={() => { updateDropdowns(cookie, !dropdowns[cookie]) }} /> :
                      <PlusIcon className='w-8 h-8' onClick={() => { updateDropdowns(cookie, !dropdowns[cookie]) }} />
                    }
                  </button>
                  <h3 className='ml-2 text-xl uppercase tracking-wider'>{cookieTranslation('cookie_' + cookie)}</h3>
                </div>
                <div className='cookieSwitch w-16 h-9 relative inline-block focus:border-2 focus:border-black' onClick={() => { updateSlider(cookie, !sliders[cookie]) }}>
                  <input type='checkbox' checked={sliders[cookie]} readOnly />
                  <span className='slider focus:border-2 focus:border-black' />
                </div>
              </div>
            {dropdowns[cookie] && <p className='px-8 mt-4 text-sm'>{cookieTranslation('cookie_' + cookie + '_description')}</p>}
            </div>
          )
        })}
        </div>
