...
export default async function Header ({ params: { language } }: { params: { language: string } }) {
  ...
  return (
    <header className='w-full h-24 text-white fixed bg-transparent backdrop-blur'>
      <div className='w-full h-full flex justify-between'>
        <div className='w-full h-full flex justify-between md:justify-start items-center'>
          ...
          <div className='w-full 2xl:w-1/2 h-full ml-16'>
              ...
              {headerElements.map((element: HeaderElement, index: number) => {
                return (
                  <li key={index} className='w-28 h-full'>
                    <Link 
                    href={element.link} 
                    className='w-full h-full grid place-items-center border-b-2 border-transparent hover:border-yellow-400' 
