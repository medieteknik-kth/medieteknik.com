...
def get_all_committees(provided_languages: list[str]) -> Dict[str, any]:
    """
    ...
    Returns:
        list[Committee]: A list of committee dictionaries.
    """
    committees: List[Committee] = Committee.query.all()

    return [committee_dict for committee in committees if (committee_dict := committee.to_dict(provided_languages)) is not None]


def get_committee_by_title(title: str, provided_languages: list[str], committee_settings: CommitteeSettings | None = None) -> Dict[str, any]:
    """
    ...
    Returns:
        Committee: A committee dictionary.
    """
    ...
    committee: Committee = Committee.query.get(translation.committee_id)

    committee_dict = committee.to_dict(provided_languages)

    if committee_settings.include_positions:
        committee_positions: List[CommitteePosition] = CommitteePosition.query.filter_by(committee_id=committee.committee_id).all()

        committee_dict['positions'] = [position.to_dict(provided_languages) for position in committee_positions]
